...
if room = r_main_menu{
	...
	var _originx = view_w/2,
		_originy = view_h/2;
	scribble_draw(_originx-40,_originy/2,"[wave]CRAFT" );


draw_sprite_ext(s_textlogo, image_index, _originx,_originy/2,1,1,0,c_white,1);
	...
	for(var i = 0; i < ds_list_size(page); i++){
		...
		if selected = i{
			scribble_draw_set_transform(1,1,0)
			_string = "[c_gum][wave]"+_string;
