...
uniform sampler2D fire_color;
uniform sampler2D aqua_color;
uniform sampler2D fuchsia_color;
uniform sampler2D yellow_color;
...
void main()
{
   ...
   // vec4 finalcol = vec4(mix(main.rgb, main.rgb*light.rgb*mult, light.rgb), 1.0);
    ...
    //finalcol.rgb = vec4(1.0);//mix(finalcol.rgb, finalcol.rgb*darkness, 1.-light.rgb);

    //Output the result.
    gl_FragColor = vec4(1.0);// finalcol;
} 