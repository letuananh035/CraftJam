...
{
    return vec2(position.x, position.y + amplitude*0.3*sin(frequency*characterIndex + speed*u_fTime*0.5));
}
...
{
    return rotate(position, centre, angle*sin(frequency*u_fTime*0.5));
}
...
{
    float adjustedScale = 1.0 + scale_*(0.5 + 0.25*sin(frequency*(250.0*characterIndex + u_fTime)));
...
vec2 shake(vec2 position, float characterIndex, float magnitude, float speed)
{
    ...
    return position + magnitude*merge*(2.0*delta-1.0)*0.15;
}
